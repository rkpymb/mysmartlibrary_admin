...
import EditCourseChaptersmodal from '../components/Edit/EditCourseChaptersmodal'
...
function DashboardCrypto({ DataSlug }) {
    ...
    useEffect(() => {
        ...
        const handleSubmit = async () => {
            ...
            const data = await fetch("/api/V3/List/CourseChaptersList", {
                ...
                .then((parsed) => {
                    if (parsed.ReqD.CoursesChapters) {
                        setRetdata(parsed.ReqD.CoursesChapters)
                        setInitialData(parsed.ReqD.CoursesChapters)
                        setIsLoading(false)
                    }
    ...
    return (
        <>
            ...
            <ToastContainer />
            <Head>
                <title>Course Chapters : {DataSlug}</title>
            </Head>
                ...
                <div className={MYS.TitleWithBackHeader}>
                    <div className={MYS.TitleWithBackHeaderA}>
                        ...
                        <div>
                            {!isLoading ?
                                <div>
                                    <span>Course Chapters : <span>{FilterText}</span> ({Retdata.length})</span>
                                </div>
                                : <div>
                        ...
                        {!isLoading ?
                            <div className={MYS.Topbtnbox}>
                                ...
                                <div className={MYS.Topbtnboxbtn}>
                                    <AddCourseChapters pid={DataSlug} />
                                </div>
                                            ...
                                            >
                                                ...
                                                <TableCell>
                                                    ...
                                                    <div className={MYS.Flexbtnbox}>
                                                        <div style={{ minWidth: '10px' }}></div>
                                                        <Button size='small' variant="outlined" onClick={() => router.push(`/CourseChapterField/${item._id}/${item.pid}/${1}`)}>
                                                            View Chapter
                                                        </Button>
                                                        <div style={{ minWidth: '10px' }}></div>
                                                        <EditCourseChaptersmodal Courseid={DataSlug} Chdata={item} />
