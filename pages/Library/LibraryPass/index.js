...
import EditPass from '../../components/Library/Edit/EditPass'
...
import AddPass from '../../components/Library/Add/AddPass'
...
function DashboardCrypto() {
  ...
  const GetData = async () => {
    ...
    const data = await fetch("/api/V3/Library/Passes/AllLibrayPasses", {
      method: "POST",
      headers: {
      ...
      .then((parsed) => {
        console.log(parsed.ReqD.PassList)

        if (parsed.ReqD.PassList) {
          setInitialData(parsed.ReqD.PassList)

          setRetdata(parsed.ReqD.PassList)
          setIsLoading(false)
  ...
  const DeletePass = async (e) => {
    ...
    if (confirm(text) == true) {
      ...
      const data = await fetch("/api/V3/Library/Passes/DeletePass", {
        method: "POST",
        headers: {
          'Content-type': 'application/json'
        },
        ...
        .then((parsed) => {
          ...
          if (parsed.ReqD.done) {
            ...
            router.push('/Library/LibraryPass')
          } else {
  ...
  return (
    <>
      <Head>
        <title>Library Subscription Passes
        </title>
      </Head>
        ...
        <div className={MYS.TitleWithBackHeader}>
          <div className={MYS.TitleWithBackHeaderA}>
            ...
            <div>
              {!isLoading ?
                <div>
                  <span>Subscription Passes : <span>{FilterText}</span> ({Retdata.length})</span>
                </div>
                : <div>
          ...
          <div className={MYS.stickytableBox} >
            <TableContainer className={MYS.stickytable}>
              <Table stickyHeader aria-label="sticky table">
                <TableHead>
                  <TableRow>
                    <TableCell>Subscription Passes</TableCell>
                    <TableCell>Pass Details</TableCell>
                      ...
                      return <TableRow hover key={item._id}>
                        <TableCell>
                          ...
                          <div style={{ width: 200 }}>
                            ...
                            <div>
                              <span style={{ fontSize: 12 }}>Pass ID  : {item.passid}</span>
                            </div>
                            <div>
                              <span style={{ fontSize: 12 }}>Validity  : {item.Validity} Days</span>
                            </div>
