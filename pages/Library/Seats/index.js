...
import AddSeat from '../../components/Library/Add/AddSeat'
...
function DashboardCrypto() {
  ...
  const GetData = async () => {
    ...
    const data = await fetch("/api/V3/Library/Seats/AllSeats", {
      method: "POST",
      headers: {
      ...
      .then((parsed) => {
        console.log(parsed.ReqD.SeatsList)

        if (parsed.ReqD.SeatsList) {
          setInitialData(parsed.ReqD.SeatsList)

          setRetdata(parsed.ReqD.SeatsList)
          setIsLoading(false)
  ...
  const DeleteShift = async (e) => {
    ...
    if (confirm(text) == true) {
      ...
      const data = await fetch("/api/V3/Library/Seats/DeleteSeat", {
        method: "POST",
        headers: {
          'Content-type': 'application/json'
        },
  ...
  const UpdateShift = async () => {
    if (title !== '') {
      ...
      const data = await fetch("/api/V3/Library/Seats/UpdateSeat", {
        method: "POST",
        headers: {
          'Content-type': 'application/json'
        },
        ...
        <div className={MYS.TitleWithBackHeader}>
          <div className={MYS.TitleWithBackHeaderA}>
            ...
            <div>
              {!isLoading ?
                <div>
                  <span>Library Seat : <span>{FilterText}</span> ({Retdata.length})</span>
                </div>
                : <div>
                      ...
                      return <TableRow hover key={item._id}>
                        <TableCell>
                          ...
                          <div style={{ width: 200 }}>
                            ...
                            <div>
                              <span style={{ fontSize: 12 }}>Seat ID  : {item.SeatCode}</span>
                            </div>
                            <div>
                              <span style={{ fontSize: 12 }}>Row Number  : {item.Row}</span>
                            </div>
                            <div>
                              <span style={{ fontSize: 12 }}>Facing Side  : {item.Facing}</span>
                            </div>
