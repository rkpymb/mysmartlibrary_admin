...
import EditTSChaptersmodal from '../components/Edit/EditTSChaptersmodal'
...
function DashboardCrypto({ DataSlug }) {
    ...
    useEffect(() => {
        ...
        const handleSubmit = async () => {
            ...
            const sendUM = { tsid: dataid, JwtToken: Contextdata.JwtToken }
            const data = await fetch("/api/V3/List/TSChaptersList", {
                ...
                .then((parsed) => {
                    if (parsed.ReqD.AllChapters) {
                        setRetdata(parsed.ReqD.AllChapters)
                        setInitialData(parsed.ReqD.AllChapters)
                        setIsLoading(false)
                    }
    ...
    return (
        <>
            ...
            <ToastContainer />
            <Head>
                <title>Test Series Chapters : {DataSlug}</title>
            </Head>
                ...
                <div className={MYS.TitleWithBackHeader}>
                    <div className={MYS.TitleWithBackHeaderA}>
                        ...
                        <div>
                            {!isLoading ?
                                <div>
                                    <span>Test Series Chapters : <span>{FilterText}</span> ({Retdata.length})</span>
                                </div>
                                : <div>
                                            ...
                                            >
                                                ...
                                                <TableCell>
                                                    <div className={MYS.Flexbtnbox}>
                                                        <div style={{ minWidth: '10px' }}></div>
                                                        <Button size='small' variant="outlined" onClick={() => router.push(`/TSChapterQues/${item._id}`)} >
                                                            View Chapter
                                                        </Button>
                                                        <div style={{ minWidth: '10px' }}></div>
                                                        <EditTSChaptersmodal tsid={DataSlug} Chdata={item} />
