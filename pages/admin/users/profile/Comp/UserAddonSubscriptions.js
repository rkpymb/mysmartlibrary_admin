...
const LibrarySubscriptions = ({ ProfileData }) => {
    ...
    const GetData = async () => {
        ...
        try {
            const response = await fetch("/api/V3/Admin/Users/UserAddonSubscriptions", {
                method: "POST",
                headers: {
            ...
            const data = await fetch("/api/V3/Admin/AddonSubs/DeleteAddonSubscription", {
                ...
                .then((parsed) => {
                    ...
                    if (parsed.ReqD.done) {
                        ...
                        router.push(`/admin/users/profile/${ProfileData.mobile}/${ProfileData._id}/2`)
                    } else {
    ...
    return (
        <div>
            <div className={MYS.BoxTitle}>Total Addons ({AllData})</div>
            <InfiniteScroll
