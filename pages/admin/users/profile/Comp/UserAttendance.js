...
const LibrarySubscriptions = ({ ProfileData }) => {
    ...
    const GetData = async () => {
        ...
        try {
            const response = await fetch("/api/V3/Admin/Users/UserAttendance", {
                method: "POST",
                headers: {
            ...
            const data = await fetch("/api/V3/Admin/LibraryAttendance/DeleteAtt", {
                ...
                .then((parsed) => {
                    ...
                    if (parsed.ReqD.done) {
                        ...
                        router.push(`/admin/users/profile/${ProfileData.mobile}/${ProfileData._id}/0`)
                    } else {
    ...
    return (
        <div>
            <div className={MYS.BoxTitle}>Absent ({AllData})</div>
            <InfiniteScroll
                    ...
                    {ReqData.map((item, index) => {
                        return <div hover key={index} className={MYS.UserItemMain}>
                            ...
                            <div className={MYS.UserItemDescB}>
                                <span style={{ fontWeight: 500 }}>Date : {item.date}</span>
