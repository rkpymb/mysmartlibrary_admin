...
export default function ScrollDialog({chapterid,pid}) {
    ...
    const AddVideo = async (e) => {
        ...
        const data = await fetch("/api/V3/Add/addlivelecture", {
            method: "POST",
            headers: {
            ...
            .then((parsed) => {
                ...
                if (parsed.ReqData.videodata) {
                    alert('Live Session Addded Successfully')
                    setOpen(false)
                    router.push(`/CourseChapterField/${chapterid}/${pid}/2`)
                } else {
    ...
    return (
        <div>
            ...
            >
                <DialogTitle id="scroll-dialog-title">Add Live Lecture</DialogTitle>
