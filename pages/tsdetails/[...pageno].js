...
import EditTSmodal from '../components/Edit/EditTSmodal'
import AddTSEducator from '../components/Add/AddTSEducator'
...
function DashboardCrypto({ DataSlug }) {
    ...
    const GetData = async () => {
        ...
        const data = await fetch("/api/V3/Data/GetTSData", {
            method: "POST",
            headers: {
    ...
    const GetEduList = async () => {
        ...
        const data = await fetch("/api/V3/List/EDUbyTS", {
            method: "POST",
            headers: {
                'Content-type': 'application/json'
            },
    ...
    const RemoveEducator = async (e) => {
        ...
        if (confirm(text) == true) {
            ...
            const data = await fetch("/api/V3/Delete/RemoveEduFromTS", {
                method: "POST",
                headers: {
                    'Content-type': 'application/json'
                },
                ...
                .then((parsed) => {
                    ...
                    if (parsed) {
                        ...
                        if (parsed.ReqData.done) {
                            ...
                            router.push(`/tsdetails/${DataSlug}`)
                        } else {
    ...
    return (
        <>
            ...
            <ToastContainer />
            <Head>
                <title>Test Series : {CourseMainData && CourseMainData.title} </title>
            </Head>
                ...
                <div className={MYS.TitleWithBackHeader}>
                    <div className={MYS.TitleWithBackHeaderA}>
                        ...
                        <div>
                            ...
                            {!Loading ?
                                <div>
                                    <span>Test Series : <span>{CourseMainData.title}</span></span>
                                </div>
                                : <div>
                        ...
                        {!Loading ?
                            <div className={MYS.Topbtnbox}>
                                ...
                                <div className={MYS.Topbtnboxbtn}>
                                    ...
                                    <EditTSmodal Tsdata={CourseMainData} />
                                </div>
                                <div className={MYS.Topbtnboxbtn}>
                                    <AddTSEducator CourseMainData={CourseMainData} />
                                </div>
                                <div style={{ minWidth: '0px' }}></div>
                                <div className={MYS.Topbtnboxbtn}>
                                    <Button
                                        onClick={() => router.push(`/TSChapters/${CourseMainData._id}`)}
                                        size="small"
                                        variant="outlined"
